...
print("="*80)
print("OASIS-1: Converting PROCESSED Analyze 7.5 to BIDS")
print("="*80)
...
for i, subject in enumerate(subjects, 1):
    ...
    # Find PROCESSED file (should be 1 per subject)
    ...
    try:
        ...
        json_data = {
            ...
            "ProcessingNote": "PROCESSED/MPRAGE/SUBJ_111 - N4 bias corrected and oriented"
        }
...
print(f"\nAge statistics:")
age_data = pd.to_numeric(participants_df['age'], errors='coerce')
...
print(f"\nCDR distribution:")
cdr_counts = participants_df['cdr'].value_counts().sort_index()
for cdr, count in cdr_counts.items():
    if cdr != 'n/a':
        print(f"  CDR {cdr}: {count} ({count/len(participants_df)*100:.1f}%)")

print(f"\nMMSE statistics:")
mmse_data = pd.to_numeric(participants_df['mmse'], errors='coerce')
print(f"  Range: {mmse_data.min():.0f} - {mmse_data.max():.0f}")
print(f"  Mean: {mmse_data.mean():.1f} ± {mmse_data.std():.1f}")

# Count healthy controls (CDR=0, age>=45)
healthy_45plus = participants_df[
    (pd.to_numeric(participants_df['cdr'], errors='coerce') == 0) &
    (pd.to_numeric(participants_df['age'], errors='coerce') >= 45)
]

print(f"\nHealthy controls (CDR=0, age ≥45): {len(healthy_45plus)} subjects")

print("\n" + "="*80)
print("OASIS-1 conversion complete!")
print("="*80)
print("\n⚠ NOTE: Used PROCESSED files (N4 corrected, correct orientation)")
print("         NOT raw files which have incorrect orientation")
